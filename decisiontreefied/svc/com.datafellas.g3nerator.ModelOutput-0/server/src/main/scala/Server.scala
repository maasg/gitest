package io.kensu.decisiontreefied_com.datafellas.g3nerator.modeloutput_0.server.impl
...
def tellCatalog():Unit = new Thread { override def run:Unit = {
  ...
  h match {
    case Some(host) =>
      ...
      val url = "/adalog/output/service?uuid=fdeed9c0-3bed-4fc5-b923-ef788b8b7d80&tpe=model&variable=model&host=${host}&port=58903"
...
val currentProjectJars = Array[String]( "io.kensu.decisiontreefied_com.datafellas.g3nerator.modeloutput_0.common-0.0.1-SNAPSHOT.jar" , "io.kensu.decisiontreefied_com.datafellas.g3nerator.modeloutput_0.server-0.0.1-SNAPSHOT.jar" ).map{j => new java.io.File(libDir, j).getAbsolutePath}
val sparkLibDir = new java.io.File(".", "spark-lib")
val fromProjectJars = Array[String]( "commons-collections-3.2.1.jar" , "jackson-mapper-asl-1.9.13.jar" , "avro-1.7.7.jar" , "avro-compiler-1.7.7.jar" , "avro-ipc-1.7.7.jar" , "commons-lang-2.6.jar" , "jackson-core-asl-1.9.13.jar" , "commons-compress-1.4.1.jar" , "velocity-1.7.jar" , "slf4j-api-1.6.4.jar" , "paranamer-2.3.jar" , "netty-3.4.0.Final.jar" , "snappy-java-1.0.5.jar" , "xz-1.0.jar" ).map{j => new java.io.File(sparkLibDir, j).getAbsolutePath}
...
val input = org.apache.spark.mllib.linalg.Vectors.dense(ds)
 ...
 val output = 3.1415
  ...
  val d = new io.kensu.decisiontreefied_com.datafellas.g3nerator.modeloutput_0.model.Domain()
   d.setOutput(output) 
