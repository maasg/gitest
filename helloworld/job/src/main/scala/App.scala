...
sparkConf.set("spark.app.name", sparkConf.get("spark.app.name", "helloworld"))

// Set project Jars

val libDir = new java.io.File(".", "lib")
val currentProjectJars = Array[String]( "com.example-helloworld.helloworld-0.0.1-SNAPSHOT.jar" ).map{j => new java.io.File(libDir, j).getAbsolutePath}
