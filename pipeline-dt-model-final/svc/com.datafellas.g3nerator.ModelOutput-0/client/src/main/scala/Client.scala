...
import com.example.pipeline_dt_model_final_com.datafellas.g3nerator.modeloutput_0.server.ServiceRequest
...
object Client {
  ...
  def serviceInfo: (String,Int) = {
    ...
    def askServiceToCatalog(): Try[(String, Int)] = {
      val path = "/adalog/output/service?uuid=bbe7efb6-db5f-483f-abe7-341e536f0b34&tpe=model&variable=model"
  ...
  def main(args:Array[String]):Unit = {
    val (_, client) = from("localhost")
    val req = new ServiceRequest (101 , false, false, 1, 2, 3, "2650", "cs_k", "code-0")
    val prediction = client.predict(req)
