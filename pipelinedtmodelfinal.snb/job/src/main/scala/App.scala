...
sparkConf.set("spark.app.name", sparkConf.get("spark.app.name", "pipelinedtmodelfinal.snb"))

// Set project Jars

val libDir = new java.io.File(s"/usr/share/pipelinedtmodelfinal.snb", "lib")
val currentProjectJars = Array[String]( "io.kensu.dtmodel-pipelinedtmodelfinal.snb.pipelinedtmodelfinal.snb-0.0.1-SNAPSHOT.jar" ).map{j => new java.io.File(libDir, j).getAbsolutePath}
val sparkLibDir = new java.io.File(s"/usr/share/pipelinedtmodelfinal.snb", "spark-lib")
