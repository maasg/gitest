...
sparkConf.set("spark.app.name", sparkConf.get("spark.app.name", "pipelinedtmodelfinal"))

// Set project Jars

val libDir = new java.io.File(s"/usr/share/pipelinedtmodelfinal", "lib")
val currentProjectJars = Array[String]( "io.kensu.dtpipeline-pipelinedtmodelfinal.pipelinedtmodelfinal-0.0.1-SNAPSHOT.jar" ).map{j => new java.io.File(libDir, j).getAbsolutePath}
